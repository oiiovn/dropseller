...
class TransactionObserver
{
    ...
    public function created(Transaction $tran)
    {
        ...
        // 1) Ưu tiên tìm referral_code trong description trước (để tránh lỗi cộng nhầm)
        if ($desc) {
            $tokens = preg_split('/[^A-Za-z0-9]+/', strtoupper($desc), -1, PREG_SPLIT_NO_EMPTY);
            $tokens = array_values(array_unique($tokens));

            if (!empty($tokens)) {
                // Tìm user theo referral_code trong description
        ...
        // 2) Fallback: Ưu tiên theo account_number nếu không tìm thấy trong description
        if ($acc) {

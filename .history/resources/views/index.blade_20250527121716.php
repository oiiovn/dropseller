    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '#openNapTienModal', function () {
            ...
            $.get('{{ route("naptien") }}?amount=' + amount, function (data) {
                ...
                // Đảm bảo delay nhỏ để DOM thêm modal xong
                setTimeout(() => {
        ...
        // Khi một modal hiển thị → tăng z-index
        $(document).on('show.bs.modal', '.modal', function () {
            const $modal = $(this);
            const zIndex = modalLevel;
            modalLevel += 10;

            $modal.css('z-index', zIndex);
            const $backdrop = $('.modal-backdrop:not(.modal-stack-adjusted)').last();
            $backdrop.css('z-index', zIndex - 1).addClass('modal-stack-adjusted');
        });

        // Khi modal bị ẩn → giảm z-index stack
...
<style>
    ...
    .modal-backdrop.modal-stack-adjusted {
        position: fixed;
    }
</style>

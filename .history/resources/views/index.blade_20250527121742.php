...
<script>
    $(document).ready(function () {
        ...
        // Khi mở modal nạp tiền → hạ modal âm xuống
        $(document).on('shown.bs.modal', '#napTienModal', function () {
            const $negativeModal = $('#negativeBalanceModal');

            if ($negativeModal.hasClass('show')) {
                $negativeModal.addClass('behind').css('z-index', 1040); // đẩy modal âm xuống
            }
        });

        // Khi đóng modal nạp tiền → khôi phục modal âm
        $(document).on('hidden.bs.modal', '#napTienModal', function () {
            const $negativeModal = $('#negativeBalanceModal');

            if ($negativeModal.length) {
                $negativeModal.removeClass('behind').css('z-index', '');
            }
        });
    });
</script>

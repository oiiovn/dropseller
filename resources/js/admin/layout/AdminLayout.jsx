...
import { ADMIN_NAV } from "../../shared/lib/navigation";
...
export default function AdminLayout({ children }) {
    ...
    const roleNames = Array.isArray(user?.role_names) ? user.role_names : ["admin"];
    ...
    return (
        <div className="admin-shell flex h-dvh overflow-hidden" style={cssVars}>
            <aside
            ...
            >
                <div className={`h-full w-64 overflow-y-auto p-4 ${sidebarCollapsed ? "lg:invisible" : ""}`}>
                    <div className="mb-6">
                        <p className="text-lg font-bold">Himawari Admin</p>
                        <p className="text-xs opacity-60">Quản trị hệ thống</p>
                    </div>
                    <GroupedSidebarNav menu={ADMIN_NAV} roleNames={roleNames} />
                </div>
            </aside>
            ...
            {mobileMenuOpen && (
                <>
                    ...
                    <aside className="crm-sidebar fixed inset-y-0 left-0 z-50 w-72 overflow-y-auto border-r p-4 shadow-xl lg:hidden">
                        ...
                        <GroupedSidebarNav menu={ADMIN_NAV} roleNames={roleNames} onNavigate={closeMobileMenu} />
                    </aside>
                </>
            )}
